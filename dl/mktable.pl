...
for $per (@sall) {
    ...
    my $metalink;
    if($$per{'re'} ne "-") {
        $mirror="<a href=\"http://curl.haxx.se/latest.cgi?curl=$$per{'re'}\" type=\"text/html\" title=\"download mirrors\">";
        $metalink="<a href=\"http://curl.haxx.se/metalink.cgi?curl=$$per{'re'}\" type=\"application/metalink4+xml\">" .
