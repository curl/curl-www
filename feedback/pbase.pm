...
# v1.14 2003-oct-17 Kjell Ericson - Added append to open.
# ==============================
...
sub modify
{
    ...
    if ($dataindex == -1) {
        if ($id eq "") {
            ...
            $dataindex=$#{$self->{Database}}+1;
            $self->{database_id}{$id}=$dataindex;
            $self->{selectionlist}[$self->{selection}][$self->size()]=$dataindex;
        }
    }
    ...
    if ($returnval==0) {
        ...
        while (<$fh>) {
            if (m/^-/) {
                ($val, $id)=split(" ", $_);
                $id=~s/[\n\r ]//g;
                $curritem=$self->{database_id}{$id};
                if ($curritem eq "") {
                    $items++;
                    $curritem=$items;
                    $self->{Database}[$curritem]{"__id"}=$id;
                    $self->{database_id}{$id}=$curritem;
                }
            } elsif ($curritem>=0) {
                if (m/:[0-9]*:/) {
                    ...
                    if (!$self->{use_template} || exists($self->{template}{$name})) {
                        $self->{Database}[$curritem]{$name}=$result;
                    }
                }
            }
        }
