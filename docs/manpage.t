...
<p class="level1">This option requiures that the library was built with kerberos4 support. This is not very common. Use <span Class="emphasis">--version</span> to see if your version supports it. 
<p class="level1">If this option is used several times, the last one will be used. 
...
<p class="level1">If you are also using the <span Class="emphasis">--speed-limit</span> option, that option will take precedence and might cripple the rate-limiting slightly, to help keeping the speed-limit logic working. 
...
<p class="level1">(HTTP/HTTPS) If the server reports that the requested page has a different location (indicated with the header line Location:) this flag will let curl attempt to reattempt the get on the new place. If used together with -i or -I, headers from all requested pages will be shown. If authentication is used, curl will only send its credentials to the initial host, so if a redirect takes curl to a different host, it won't intercept the user+password. See also <a class="emphasis" href="#--location-trusted">--location-trusted</a> on how to change this. 
...
<p class="level1">This option requiures that the library was built with GSSAPI support. This is not very common. Use <span Class="emphasis">--version</span> to see if your version supports GSS-Negotiate. 
<p class="level1">If this option is used several times, the following occurrences make no difference. 
...
<p class="level1">This option requiures that the library was built with SSL support. Use <span class="emphasis">curl --version</span> to see if your version supports NTLM. 
<p class="level1">If this option is used several times, the following occurrences make no difference. 
...
<p class="level1">See also the --create-dirs option to create the local directories dynamically. 
...
<p class="level1">Tells curl to use NTLM authentication when communicating with the given proxy. 
...
<p class="level1">When an HTTP proxy is used, this option will cause non-HTTP protocols to attempt to tunnel through the proxy instead of merely using it to do HTTP-like operations. The tunnel approach is made with the HTTP proxy CONNECT request and requires that the proxy allows direct connect to the remote port number curl wants to tunnel through to. 
...
<p class="level1">
<p class="level1">If this option is used several times, the last one will be used. Disable the use of PORT with <a class="emphasis" href="#--ftp-pasv">--ftp-pasv</a>. 
...
<p class="level1">Enables a full trace dump of all incoming and outgoing data, including descriptive information, to the given output file. Use "-" as filename to have the output sent to stdout. 
<p class="level1">This is very similar to --trace, but leaves out the hex part and only shows the ASCII part of the dump. It makes smaller output that might be easier to read for untrained humans. 
<p class="level1">If this option is used several times, the last one will be used. (Added in 7.9.7) 
<p class="level0"><a name="-u--user"></a><span class="nroffip">-u/--user &lt;user:password&gt;</span> 
<p class="level1">Specify user and password to use when fetching. Read the MANUAL for detailed examples of how to use this. If no password is specified, curl will ask for it interactively. 
<p class="level1">You can also use the --digest option to enable Digest authentication when communicating with HTTP 1.1 servers. 
<p class="level1">If this option is used several times, the last one will be used. 
<p class="level0"><a name="-U--proxy-user"></a><span class="nroffip">-U/--proxy-user &lt;user:password&gt;</span> 
<p class="level1">Specify user and password to use for Proxy authentication. If no password is specified, curl will ask for it interactively. 
<p class="level1">If this option is used several times, the last one will be used. 
...
<p class="level1">This option may be used any number of times. To control where this URL is written, use the <span Class="emphasis">-o</span> or the <span Class="emphasis">-O</span> options. 
...
<p class="level1">The third line (starts with "Features:") shows specific features libcurl reports to offer. 
<p class="level1">
...
<p class="level1">Set maximum number of redirection-followings allowed. If -L/--location is used, this option can be used to prevent curl from following redirections "in absurdum". 
<p class="level1">If this option is used several times, the last one will be used. 
...
<p class="level1">HTTP page not retrieved. The requested url was not found or returned another error with the HTTP error code being 400 or above. This return code only appears if --fail is used. 
